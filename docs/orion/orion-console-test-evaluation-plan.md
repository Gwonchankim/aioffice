...
- Codex는 정확히 한 번 `exec --json --sandbox read-only --cd <repo> --output-schema <file> --model <approved> --ephemeral --ignore-user-config --ignore-rules -`를 실행하고, Claude는 정확히 한 번 `--print --output-format stream-json --verbose --json-schema <serialized JSON string> --model <approved(default sonnet)> --effort low --permission-mode dontAsk --allowedTools Read,Glob,Grep --disallowedTools Bash,Edit,Write,WebFetch,WebSearch --no-chrome --no-session-persistence --strict-mcp-config --mcp-config {"mcpServers":{}} --disable-slash-commands --max-budget-usd 0.50`를 실행한다(`--max-turns`는 claude 2.1.156이 지원하지 않아 사용하지 않으며 자동 fallback 없음). 둘 다 `shell:false`, stdin prompt, invocation별 5분 timeout이다. grant는 provider별 executable(CLI version + SHA-256 fingerprint, `--version` probe만) + argv/schema/prompt/repository policy projection을 strict schema로 결속하고, run은 argv model을 grant에서만 취하며(다른 run-time model env는 0-spawn conflict), spawn 직전마다 executable을 재fingerprint(불일치 시 0 spawn)하고, spawn 직전에 durable spawn-attempt marker를 기록한다. 저장소 밖 durable ledger가 crash·재실행 추가 spawn을 0회로 만들고, repository 변경 시 남은 provider를 0 spawn으로 즉시 중단한다.
