...
grant은 provider별 executable을 resolve·SHA-256 fingerprint(+`--version` probe, 0 model call)하고 argv/schema/prompt/repository policy projection과 함께 strict schema로 결속하며, grant CLI는 raw authorization id 대신 one-way `authorizationIdHash`만 출력한다. run은 argv model을 grant에서만 취하고(다른 run-time model env는 `MODEL_BINDING_CONFLICT` 0-spawn), policy projection을 재계산·대조하며, spawn 직전마다 executable을 재fingerprint(`EXECUTABLE_BINDING_MISMATCH` 시 0 spawn)하고 durable spawn-attempt marker를 먼저 기록한다. Codex argv에 `--ephemeral --ignore-user-config --ignore-rules`, Claude argv에 `--no-chrome --no-session-persistence --strict-mcp-config --mcp-config {"mcpServers":{}} --disable-slash-commands`가 포함된다(`--max-turns`는 미지원이라 제외). repository 변경 시 남은 provider는 0 spawn `security_halt`이며, 이 실행이 만든 temp dir는 `finally`에서 정리된다. evidence는 provider별 `reservedCount`/`spawnAttemptCount`/`invocationCount`(= 누적 spawn attempt)와 `cleanup` 상태를 포함하고 raw executable path는 남기지 않는다(fingerprint hash만).
