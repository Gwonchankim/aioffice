...
## Rev4 — final resolution of P2 round-3 blockers (each adopts the reviewer's recommended fix verbatim)

- **D1 (fixes B5/R6+R4 count contradiction).** `invocationCount` is ALWAYS assembled from a fresh `usage(authId)` slot-marker count at envelope-assembly time — including `run_claim_denied`, `authorization_exhausted`, and every `invocation_*` stage. It is `0` only when it falls out naturally (no markers) — i.e. `authorization_missing` (no authId), `ledger_unsafe`/`grant_corrupt`/`grant_missing` (ledger not safely readable / no grant dir). No stage forces a constant. A rerun that is claim-denied after a prior 1 Codex + 1 Claude therefore reports the real cumulative counts (1 and 1), not 0.
- **D2 (fixes R1 identity validation).** Single shared boundary helper `composeFrameIdentity(...parts): string | undefined` joins parts with `:` and validates the FINAL composite against `providerEventIdentitySchema`'s exact regex `/^[A-Za-z0-9][A-Za-z0-9._:-]{0,255}$/` (also enforcing the 256-char cap). Used for BOTH the mapping dedup id and every emitted event's `providerEventId`. Any wire component that makes the composite illegal/overlength ⇒ helper returns `undefined` ⇒ mapper returns `invalidMapping()` (never a throw downstream). Tests: illegal-char component, overlength component.
- **D3 (fixes R1 identifier-less terminal dedup).** Codex `turn.completed` mapping identity = the stable literal `turn.completed`; Claude terminal `result` identity = `result:<uuid>`. Each is deduped once per parser instance (resume uses a fresh parser). Test: duplicate `turn.completed` / duplicate `result` emits usage/result exactly once.
- **D4 (fixes R2 required delayMs).** A retry frame is emitted ONLY when both a positive `attempt` AND a non-negative delay are present (Claude `retry_delay_ms`, Codex `delay_ms`); otherwise the frame is `unknown` (ignored, counted as unknownEvent) — never a fabricated `delayMs`. Test: retry with missing delay ⇒ unknown.
- **D5 (fixes R5 grant idempotence).** Grant equality compares only the semantic projection `{schemaVersion, authorizationId, providers, options}` (canonical sorted-key JSON), EXCLUDING the generated `createdAt`; identical semantics ⇒ idempotent no-op that retains the first stored `createdAt`; any semantic difference ⇒ `PROVIDER_GRANT_CONFLICT`. Test: repeat identical CLI grant ⇒ idempotent; changed model ⇒ conflict.
